#!/usr/bin/env python3
"""
miner.py — Files everything into the ship.

Reads multipass.yaml from the project directory to know the wing + rooms.
Routes each file to the right room based on content.
Stores verbatim chunks as crates. No summaries. Ever.
"""
...
CHUNK_SIZE = 800  # chars per crate
...
def load_config(project_dir: str) -> dict:
    """Load multipass.yaml from project directory (falls back to mempal.yaml)."""
    import yaml

    config_path = Path(project_dir).expanduser().resolve() / "multipass.yaml"
    if not config_path.exists():
        ...
        if legacy_path.exists():
            ...
            print(f"ERROR: No multipass.yaml found in {project_dir}")
            print(f"Run: multipass init {project_dir}")
            sys.exit(1)
...
def chunk_text(content: str, source_file: str) -> list:
    """
    Split content into crate-sized chunks.
...
# =============================================================================
# SHIP — ChromaDB operations
# =============================================================================


def get_collection(ship_path: str):
    os.makedirs(ship_path, exist_ok=True)
    client = chromadb.PersistentClient(path=ship_path)
    try:
        return client.get_collection("multipass_crates")
    except Exception:
        return client.create_collection("multipass_crates")


def file_already_mined(collection, source_file: str) -> bool:
    """Fast check: has this file been filed before and is unchanged?

    Compares the stored mtime in crate metadata against the file's current
...
def add_crate(
    ...
    """Add one crate to the ship."""
    crate_id = f"crate_{wing}_{room}_{hashlib.md5((source_file + str(chunk_index)).encode(), usedforsecurity=False).hexdigest()[:16]}"
    try:
        metadata = {
            "wing": wing,
            "room": room,
...
def process_file(
    ...
    """Read, chunk, route, and file one file. Returns (crate_count, room_name)."""
    ...
    if dry_run:
        print(f"    [DRY RUN] {filepath.name} → room:{room} ({len(chunks)} crates)")
...
def mine(
    ...
    """Mine a project directory into the ship."""
    ...
    if not dry_run:
        collection = get_collection(ship_path)
    else:
        collection = None
    ...
    for i, filepath in enumerate(files, 1):
        ...
        if crates == 0 and not dry_run:
            files_skipped += 1
        else:
            total_crates += crates
            room_counts[room] += 1
            if not dry_run:
                print(f"  ✓ [{i:4}/{len(files)}] {filepath.name[:50]:50} +{crates}")

    print(f"\n{'=' * 55}")
    print("  Done.")
    print(f"  Files processed: {len(files) - files_skipped}")
    print(f"  Files skipped (already filed): {files_skipped}")
    print(f"  Crates filed: {total_crates}")
    print("\n  By room:")
    for room, count in sorted(room_counts.items(), key=lambda x: x[1], reverse=True):
        print(f"    {room:20} {count} files")
    print('\n  Next: multipass search "what you\'re looking for"')
    print(f"{'=' * 55}\n")
...
def status(ship_path: str):
    """Show what's been filed in the ship."""
    try:
        client = chromadb.PersistentClient(path=ship_path)
    ...
    print(f"\n{'=' * 55}")
    print(f"  multipass Status — {len(metas)} crates")
    print(f"{'=' * 55}\n")
    for wing, rooms in sorted(wing_rooms.items()):
        print(f"  WING: {wing}")
        for room, count in sorted(rooms.items(), key=lambda x: x[1], reverse=True):
            print(f"    ROOM: {room:20} {count:5} crates")
        print()
    print(f"{'=' * 55}\n")
