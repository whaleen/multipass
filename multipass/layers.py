#!/usr/bin/env python3
"""
layers.py — 4-Layer Memory Stack for multipass
===================================================

Load only what you need, when you need it.

    Layer 0: Identity       (~100 tokens)   — Always loaded. "Who am I?"
    Layer 1: Essential Story (~500-800)      — Always loaded. Top moments from the ship.
...
Reads directly from ChromaDB (multipass_crates)
and ~/.multipass/identity.txt.
"""
...
class Layer0:
    ...
    Reads from ~/.multipass/identity.txt — a plain-text file the user writes.
    ...
    def __init__(self, identity_path: str = None):
        if identity_path is None:
            identity_path = os.path.expanduser("~/.multipass/identity.txt")
    ...
    def render(self) -> str:
        ...
        if os.path.exists(self.path):
            ...
            self._text = (
                "## L0 — IDENTITY\nNo identity configured. Create ~/.multipass/identity.txt"
            )
...
# ---------------------------------------------------------------------------
# Layer 1 — Essential Story (auto-generated from ship)
# ---------------------------------------------------------------------------
...
class Layer1:
    ...
    Auto-generated from the highest-weight / most-recent crates in the ship.
    ...
    def __init__(self, ship_path: str = None, wing: str = None):
        cfg = MultipassConfig()
        self.ship_path = ship_path or cfg.ship_path
        self.wing = wing

    def generate(self) -> str:
        """Pull top crates from ChromaDB and format as compact L1 text."""
        try:
            client = chromadb.PersistentClient(path=self.ship_path)
            col = client.get_collection("multipass_crates")
        except Exception:
            return "## L1 — No ship found. Run: multipass mine <dir>"

        # Fetch all crates in batches to avoid SQLite variable limit (~999)
        _BATCH = 500
        docs, metas = [], []
        offset = 0
        while True:
        ...
        # Score each crate: prefer high importance, recent filing
        scored = []
...
class Layer2:
    ...
    def __init__(self, ship_path: str = None):
        cfg = MultipassConfig()
        self.ship_path = ship_path or cfg.ship_path

    def retrieve(self, wing: str = None, room: str = None, n_results: int = 10) -> str:
        """Retrieve crates filtered by wing and/or room."""
        try:
            client = chromadb.PersistentClient(path=self.ship_path)
            col = client.get_collection("multipass_crates")
        except Exception:
        ...
        if not docs:
            ...
            return f"No crates found for {label}."

        lines = [f"## L2 — ON-DEMAND ({len(docs)} crates)"]
...
class Layer3:
    """
    Unlimited depth. Semantic search against the full ship.
    Reuses searcher.py logic against multipass_crates.
    """
...
class MemoryStack:
    """
    The full 4-layer stack. One class, one ship, everything works.
    ...
    def __init__(self, ship_path: str = None, identity_path: str = None):
        cfg = MultipassConfig()
        self.ship_path = ship_path or cfg.ship_path
        self.identity_path = identity_path or os.path.expanduser("~/.multipass/identity.txt")

        self.l0 = Layer0(self.identity_path)
        self.l1 = Layer1(self.ship_path)
        self.l2 = Layer2(self.ship_path)
        self.l3 = Layer3(self.ship_path)
    ...
    def status(self) -> dict:
        ...
        result = {
            ...
            "L1_essential": {
                "description": "Auto-generated from top ship crates",
            },
            "L2_on_demand": {
        ...
        try:
            ...
            col = client.get_collection("multipass_crates")
            count = col.count()
            result["total_crates"] = count
        except Exception:
            result["total_crates"] = 0
