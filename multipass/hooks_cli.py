"""
Hook logic for multipass — Python implementation of session-start, stop, and precompact hooks.
...
SAVE_INTERVAL = 15
STATE_DIR = Path.home() / ".multipass" / "hook_state"
...
def _maybe_auto_ingest():
    """If MULTIPASS_DIR is set and exists, run multipass mine in background."""
    multipass_dir = os.environ.get("MULTIPASS_DIR", "")
    if multipass_dir and os.path.isdir(multipass_dir):
        try:
            log_path = STATE_DIR / "hook.log"
            with open(log_path, "a") as log_f:
                subprocess.Popen(
                    [sys.executable, "-m", "multipass", "mine", multipass_dir],
                    stdout=log_f,
                    stderr=log_f,
                )
        except OSError:
...
def hook_stop(data: dict, harness: str):
    ...
    if since_last >= SAVE_INTERVAL and exchange_count > 0:
        ...
        # Optional: auto-ingest if MULTIPASS_DIR is set
        _maybe_auto_ingest()
