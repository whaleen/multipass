#!/usr/bin/env python3
"""
convo_miner.py — Mine conversations into the ship.

Ingests chat exports (Claude Code, ChatGPT, Slack, plain text transcripts).
Normalizes format, chunks by exchange pair (Q+A = one unit), files to ship.

Same ship as project mining. Different ingest strategy.
"""
...
def mine_convos(
    ...
    """Mine a directory of conversation files into the ship.
    ...
    print(f"\n{'=' * 55}")
    print("  multipass Mine — Conversations")
    print(f"{'=' * 55}")
    print(f"  Wing:    {wing}")
    ...
    collection = get_collection(ship_path) if not dry_run else None
    ...
    for i, filepath in enumerate(files, 1):
        ...
        for chunk in chunks:
            ...
            crate_id = f"crate_{wing}_{chunk_room}_{hashlib.md5((source_file + str(chunk['chunk_index'])).encode(), usedforsecurity=False).hexdigest()[:16]}"
            try:
                collection.add(
        ...
        total_crates += crates_added
        print(f"  ✓ [{i:4}/{len(files)}] {filepath.name[:50]:50} +{crates_added}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python convo_miner.py <convo_dir> [--ship PATH] [--limit N] [--dry-run]")
        sys.exit(1)
    from .config import MultipassConfig

    mine_convos(sys.argv[1], ship_path=MultipassConfig().ship_path)
