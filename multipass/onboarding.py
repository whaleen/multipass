#!/usr/bin/env python3
"""
onboarding.py — multipass first-run setup.

Asks the user:
  1. How they're using multipass (work / personal / combo)
...
Seeds the entity_registry with confirmed data so multipass knows your world
from minute one — before a single session is indexed.

Usage:
    python3 -m multipass.onboarding
    or: multipass init
"""

from pathlib import Path
from multipass.entity_registry import EntityRegistry
from multipass.entity_detector import detect_entities, scan_for_detection
...
def _ask_mode() -> str:
    _header("Welcome to multipass")
    print("""
  multipass is a personal memory system. To work well, it needs to know
...
""")
    print("  How are you using multipass?")
    print()
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help multipass distinguish project
...
def _ask_wings(mode: str) -> list:
    ...
    _hr()
    print(f"""
  Wings are the top-level categories in your memory ship.
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from multipass.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_aaak_bootstrap(
    ...
    """
    multipass_dir = Path(config_dir) if config_dir else Path.home() / ".multipass"
    multipass_dir.mkdir(parents=True, exist_ok=True)
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by multipass init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    (multipass_dir / "aaak_entities.md").write_text("\n".join(registry_lines))

    # Critical facts bootstrap (pre-ship — before any mining)
    facts_lines = [
    ...
    (multipass_dir / "critical_facts.md").write_text("\n".join(facts_lines))
...
def run_onboarding(
  ...
  multipass will check the context before treating them as person names.
...
""")
    ...
    print("\n  AAAK entity registry: ~/.multipass/aaak_entities.md")
    print("  Critical facts bootstrap: ~/.multipass/critical_facts.md")
...
def quick_setup(
    ...
    Used in tests and local support scripts.
