...
from AAAK output. It is a structured summary layer (lockers) that points to the
original verbatim content (crates). The earlier published score was from raw mode,
not AAAK mode.

Adapted for multipass: works standalone on plain text and ChromaDB crates.
No dependency on ship.py or layers.py.
...
class Dialect:
    ...
    # === PLAIN TEXT COMPRESSION (new for multipass) ===
