"""
ship_graph.py — Graph traversal layer for multipass
======================================================

Builds a navigable graph from the ship structure:
  - Nodes = rooms (named ideas)
  - Edges = shared rooms across wings (tunnels)
  - Edge types = corridors (the corridors)
...
def _get_collection(config=None):
    ...
    try:
        client = chromadb.PersistentClient(path=config.ship_path)
...
def build_graph(col=None, config=None):
    """
    Build the ship graph from ChromaDB metadata.

    Returns:
        nodes: dict of {room: {wings: set, corridors: set, count: int}}
        edges: list of {room, wing_a, wing_b, corridor} — one per tunnel crossing
    """
    if col is None:
    ...
    total = col.count()
    room_data = defaultdict(lambda: {"wings": set(), "corridors": set(), "count": 0, "dates": set()})
    ...
    while offset < total:
        ...
        for meta in batch["metadatas"]:
            ...
            if room and room != "general" and wing:
                ...
                if corridor:
                    room_data[room]["corridors"].add(corridor)
                if date:
    ...
    for room, data in room_data.items():
        nodes[room] = {
            "wings": sorted(data["wings"]),
            "corridors": sorted(data["corridors"]),
            "count": data["count"],
...
def traverse(start_room: str, col=None, config=None, max_hops: int = 2):
    ...
    Returns list of paths: [{room, wing, corridor, hop_distance}]
    """
...
def find_tunnels(wing_a: str = None, wing_b: str = None, col=None, config=None):
    ...
    These are the "corridors" — same named idea appearing in multiple domains.
    """
    nodes, edges = build_graph(col, config)
...
def graph_stats(col=None, config=None):
    """Summary statistics about the ship graph."""
    nodes, edges = build_graph(col, config)
