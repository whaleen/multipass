# multipass/ — Core Package

The Python package that powers multipass. All modules, all logic.
...
| `config.py` | Configuration loading — `~/.multipass/config.json`, env vars, defaults |
...
| `ship_graph.py` | Room-based navigation graph — BFS traversal, tunnel detection across wings |
| `mcp_server.py` | MCP server — 19 tools, AAAK auto-teach, Ship Protocol, agent diary |
...
```
User → CLI → miner/convo_miner → ChromaDB (ship)
                                     ↕
...
The ship (ChromaDB) stores verbatim content. The knowledge graph (SQLite) stores structured relationships. The MCP server exposes both to any AI tool.
