...
# 3. Returns a reason telling the AI to save structured diary + ship entries
# 4. AI does the save (topics, decisions, code, quotes → organized into ship)
# 5. Next Stop fires with stop_hook_active=true → lets AI stop normally
#
# The AI does the classification — it knows what wing/corridor/locker to use
...
#       "hooks": [{
#         "type": "command",
#         "command": "/absolute/path/to/multipass_save_hook.sh",
#         "timeout": 30
#       }]
#     }]
#   }
...
# Set MULTIPASS_DIR below if you want the hook to auto-ingest after blocking.
...
# Log for debugging (check ~/.multipass/hook_state/hook.log)
...
if [ "$SINCE_LAST" -ge "$SAVE_INTERVAL" ] && [ "$EXCHANGE_COUNT" -gt 0 ]; then
    ...
    # Optional: run multipass ingest in background if MULTIPASS_DIR is set
